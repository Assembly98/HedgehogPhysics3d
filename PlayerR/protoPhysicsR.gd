...
@export var slopeSpdLimit : float = 10
@export var SlopeRunningAngleLimit : float = 0.5
@export var GroundStickingPower : float = -1
...
func generalPhysics() -> void:
	...
	print("velLength: ", linear_velocity.length())
	...
	if abs(linear_velocity.y) > maxFallSpd:
		...
		#reducedSpd.x = keepX
		#reducedSpd.z = keepZ
		linear_velocity.y = sign(linear_velocity.y) * maxFallSpd
...
func airMovement():
	...
	if linear_velocity.y < -maxFallSpd:
		linear_velocity = Vector3(linear_velocity.x, -maxFallSpd, linear_velocity.z)
